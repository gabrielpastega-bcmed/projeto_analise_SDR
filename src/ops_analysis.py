...
def calculate_response_times(chat: Chat) -> Dict[str, Any]:
    ...
    return {
        "tme_seconds": avg_wait_time,
        "tma_seconds": tma_seconds,
        "response_count": response_count
    }

def analyze_agent_performance(chats: List[Chat]) -> Dict[str, Any]:
    """
    ...
    for chat in chats:
        ...
        agent_stats[agent_name]["chats"] += 1
    ...
    for name, stats in agent_stats.items():
        avg_tme = stats["total_tme"] / stats["response_count"] if stats["response_count"] > 0 else 0
        avg_tma = stats["total_tma"] / stats["chats"] if stats["chats"] > 0 else 0
